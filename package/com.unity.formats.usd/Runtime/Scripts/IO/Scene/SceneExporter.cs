      ...
      Transform expRoot = context.exportRoot;
            ...
            else if (mf != null && mr != null)
            {
                foreach (var mat in mr.sharedMaterials)
                {
                    ...
                    {
                        string usdPath = materialBasePath +
                        pxr.UsdCs.TfMakeValidIdentifier(mat.name); // Gaussian: pxr.UsdCs.TfMakeValidIdentifier(mat.name + "_" + mat.GetInstanceID().ToString()); //this will create a lot of same materials
                        context.matMap.Add(mat, usdPath);
                    }
                }
